...
from app.free_strategy.four_mode_snapshot import FourModeSnapshotCache
...
def _score_four_mode_first_board(
    candidate: dict[str, Any],
    feature: dict[str, Any] | None,
    quote: dict[str, Any] | None,
) -> tuple[dict[str, Any] | None, str | None]:
    """Apply the four-mode first-board intraday filter and score."""
    if not isinstance(feature, dict) or not feature.get("fresh"):
        return None, "分钟数据未就绪"
    bars = [row for row in feature.get("session_bars") or [] if isinstance(row, dict)]
    if len(bars) < 5:
        return None, "分钟数据不足5根"
    current = _finite((quote or {}).get("last_price"))
    if current is None:
        current = _finite(feature.get("last_price"))
    vwap = _finite(feature.get("session_vwap"))
    if current is None or current <= 0 or vwap is None or vwap <= 0:
        return None, "缺少当前价或VWAP"
    day_open = _finite(bars[0].get("open"))
    if day_open is None or day_open <= 0:
        return None, "缺少当日开盘价"
    quote_change = _finite((quote or {}).get("change_pct"))
    rise_pct = (
        round(quote_change * 100.0, 4)
        if quote_change is not None
        else round((current / day_open - 1.0) * 100.0, 4)
    )
    if rise_pct < 2.0 or rise_pct > 9.5:
        return None, "当前涨幅不在2%-9.5%"
    volumes = [_finite(row.get("volume")) or 0.0 for row in bars]
    latest_volume = volumes[-1]
    prior_volumes = volumes[-4:-1]
    prior_average = sum(prior_volumes) / len(prior_volumes) if prior_volumes else 0.0
    volume_ratio = latest_volume / prior_average if prior_average > 0 else 0.0
    vwap_volume_met = current > vwap and volume_ratio > 1.5

    highs = [value for value in (_finite(row.get("high")) for row in bars) if value is not None]
    lows = [value for value in (_finite(row.get("low")) for row in bars) if value is not None]
    if not highs or not lows:
        return None, "缺少当日高低点"
    day_high = max(max(highs), current)
    day_low = min(min(lows), current)
    range_value = day_high - day_low
    upper_half = range_value > 0 and (current - day_low) / range_value > 0.5

    attack_count = 0
    for index in range(max(1, len(bars) - 5), len(bars)):
        close = _finite(bars[index].get("close"))
        previous_close = _finite(bars[index - 1].get("close"))
        if close is None or previous_close is None or close <= previous_close:
            continue
        window = [value for value in volumes[max(0, index - 3):index] if value > 0]
        average = sum(window) / len(window) if window else 0.0
        if average > 0 and volumes[index] > average * 1.3:
            attack_count += 1

    score = (35 if vwap_volume_met else 0) + (25 if upper_half else 0) + (30 if attack_count >= 3 else 0)
    standard_buy = 2.0 <= rise_pct <= 8.0 and score >= 60
    strong_buy = rise_pct >= 8.0 and rise_pct <= 9.5 and current > vwap
    if not standard_buy and not strong_buy:
        return None, "盘中评分或涨幅分支不满足"
    branch = "standard" if standard_buy else "strong"
    return {
        "symbol": str(candidate.get("symbol") or "").strip().upper(),
        "static_score": _finite(candidate.get("score")),
        "score": float(score),
        ...
        "vwap_volume_met": vwap_volume_met,
        "branch": branch,
        "buy_eligible": True,
...
class LimitBoardService:
    def __init__(self, data_dir: Path, repo: Any, quote_service: Any, app_state: Any) -> None:
        ...
        self._four_mode_snapshot: FourModeSnapshotCache | None = None
        self._four_mode_snapshot_key: tuple[date, date | None] | None = None
        self._four_mode_snapshot_lock = threading.Lock()
    ...
    def four_mode_first_board_view(self) -> dict[str, Any]:
        """Return the current four-mode static first-board candidates."""
        today = cn_today()
        try:
            _latest, latest_date = self.repo.get_enriched_latest()
        except Exception:  # noqa: BLE001
            latest_date = None
        cache_key = (today, latest_date)
        try:
            with self._four_mode_snapshot_lock:
                if self._four_mode_snapshot is None or self._four_mode_snapshot_key != cache_key:
                    self._four_mode_snapshot = FourModeSnapshotCache(
                        self.repo,
                        today,
                        today,
                        _FOUR_MODE_REQUIREMENT,
                    )
                    self._four_mode_snapshot_key = cache_key
                snapshot = self._four_mode_snapshot.snapshot(today)
        except Exception as exc:  # noqa: BLE001
            logger.warning("读取四合一首板快照失败", exc_info=True)
            return {
                "provider": "four_mode_snapshot",
                "state": "unavailable",
                "as_of": None,
                "refreshed_at": None,
                "data_gaps": [str(exc) or "四合一首板快照不可用"],
                "rows": [],
            }

        static_mode = (snapshot.get("static_modes") or {}).get("sb")
        if not isinstance(static_mode, dict):
            return {
                ...
                "refreshed_at": None,
                "data_gaps": list(snapshot.get("data_gaps") or ["四合一首板快照尚未构建"]),
                "rows": [],
            }
        candidates = [
            candidate for candidate in static_mode.get("candidates") or []
            if isinstance(candidate, dict) and str(candidate.get("symbol") or "").strip()
        ]
        candidates.sort(key=lambda row: (-float(row.get("score") or 0), str(row.get("symbol"))))
        static_change_by_symbol = {
            str(item.get("symbol") or "").strip().upper(): _finite(item.get("chg_pct"))
            for item in candidates
        }

        def pending_rows() -> list[dict[str, Any]]:
            """Expose daily-screened candidates while intraday confirmation is pending."""
            ...
            for rank, candidate in enumerate(candidates, start=1):
                ...
                rows.append({
                    ...
                    "static_score": _finite(candidate.get("score")),
                    "change_pct": None,
                    "current_rise_pct": None,
                    "last_price": None,
                    "yesterday_change_pct": static_change_by_symbol.get(symbol),
                    "volume_ratio": None,
                    "vwap": None,
                    "upper_half": False,
                    "attack_count": 0,
                    "vwap_volume_met": False,
                    "branch": "pending",
                    "buy_eligible": False,
                    "tags": ["四合一首板", "待盘中确认"],
                })
        ...
        if not _is_trading_time(now):
            message = (
                "当日已收盘，下一交易日盘中再计算 VWAP 和评分"
                if _is_after_close_auction(now)
                else "当前非连续竞价时段，下一交易日盘中再计算 VWAP 和评分"
            )
            return {
                "provider": "four_mode_snapshot",
                "state": "waiting_data",
        ...
        quotes = {
            ...
            for symbol, value in (fresh_payload.get("quotes") or {}).items()
            if isinstance(value, dict)
        }
        if not quotes:
            try:
                quotes = {
                    str(value.get("symbol") or "").strip().upper(): dict(value)
                    for value in self.quote_service.get_latest_quotes(symbols)
        ...
        for candidate in candidates:
            ...
            if scored is None:
                if reason in {"分钟数据未就绪", "分钟数据不足5根", "缺少当前价或VWAP"}:
                    waiting_count += 1
                continue
            scored_rows.append(scored)
        scored_rows.sort(key=lambda row: (-float(row.get("score") or 0), -float(row.get("current_rise_pct") or 0), str(row.get("symbol"))))
        rows = []
        for rank, candidate in enumerate(scored_rows, start=1):
            ...
            rows.append({
                ...
                "static_score": _finite(candidate.get("static_score")),
                "change_pct": current_change if current_change is not None else (_finite(candidate.get("current_rise_pct")) or 0.0) / 100.0,
                "current_rise_pct": _finite(candidate.get("current_rise_pct")),
                "last_price": _finite(candidate.get("last_price")),
                "yesterday_change_pct": static_change_by_symbol.get(symbol),
                "volume_ratio": _finite(candidate.get("volume_ratio")),
                "vwap": _finite(candidate.get("vwap")),
                "upper_half": bool(candidate.get("upper_half")),
                "attack_count": int(candidate.get("attack_count") or 0),
                "vwap_volume_met": bool(candidate.get("vwap_volume_met")),
                "branch": candidate.get("branch"),
                "buy_eligible": True,
                "tags": ["四合一首板", "强势快速" if candidate.get("branch") == "strong" else "标准确认"],
            })
        state = "ready"
        data_gaps = list(static_mode.get("data_gaps") or [])
        if not scored_rows and candidates and waiting_count == len(candidates):
            state = "waiting_data"
            data_gaps = data_gaps or ["实时分钟数据尚未就绪"]
            rows = pending_rows()
        return {
            "provider": "four_mode_snapshot",
            "state": state,
            ...
            "data_gaps": data_gaps,
            "evaluated_count": len(candidates),
            "filtered_count": len(scored_rows),
            "rows": rows,
        }

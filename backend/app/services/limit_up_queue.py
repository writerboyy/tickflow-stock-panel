...
_NEW_ORDER_STATUS = {4, 12, 64, 192}
_REDUCTION_STATUS = {1, 2, 9, 16, 32, 128, 144}
...
class _D202Watcher:
    ...
    def feed(self, data: dict[str, Any], timestamp: int) -> None:
        self._timestamp = int(timestamp or 0)
        if int(data.get("isFirst") or 0) == 1:
            self._feed_snapshot(data)
        else:
            self._feed_incremental(data)
        self._finish()

    def _feed_snapshot(self, data: dict[str, Any]) -> None:
        records = data.get("records") or []
        total = int(data.get("totalCount") or len(records))
        seq = int(data.get("seq") or 0)
        self._records = [{"id": 0, "volume": 0} for _ in range(total)]
        for index, item in enumerate(records):
            position = seq + index
            if 0 <= position < total:
                self._records[position] = {"id": int(item.get("id") or 0), "volume": int(item.get("volume") or 0)}
        self._current.count = int(data.get("totalCount") or 0)
        self._current.volume = int(data.get("totalVolume") or 0)
    ...
    def _feed_incremental(self, data: dict[str, Any]) -> None:
        for item in data.get("records") or []:
            status = int(item.get("status") or 0)
            record_id = int(item.get("id") or 0)
            volume = int(item.get("volume") or 0)
            if status in _NEW_ORDER_STATUS:
                self._records.append({"id": record_id, "volume": volume})
                self._current.count += 1
                self._current.volume += volume
                self._new_add.count += 1
                self._new_add.volume += volume
                for entry in self._my_orders:
                    if entry.found_id == 0 and (entry.hand_count < 0 or entry.hand_count == volume):
                        entry.found_id = record_id
                        entry.status = 2
                        entry.front.volume = max(0, self._current.volume - volume)
                        entry.front.count = max(0, self._current.count - 1)
                    elif entry.found_id:
                        entry.back.volume += volume
                        entry.back.count += 1
                continue
            index = next((i for i, record in enumerate(self._records) if record["id"] == record_id), -1)
            if index < 0 or status not in _REDUCTION_STATUS:
                continue
            old_volume = self._records[index]["volume"]
            reduction = max(0, old_volume - volume)
            self._records[index]["volume"] = volume
            self._current.volume = max(0, self._current.volume - reduction)
            if volume == 0:
                self._current.count = max(0, self._current.count - 1)
            target = self._executed if status in {1, 9, 16, 128, 144} else self._cancelled
            target.count += 1
            target.volume += reduction
            for entry in self._my_orders:
                if not entry.found_id:
                    continue
                if record_id == entry.found_id and volume == 0:
                    entry.status = 100
                elif record_id > entry.found_id:
                    entry.back.volume = max(0, entry.back.volume - reduction)
                else:
                    entry.front.volume = max(0, entry.front.volume - reduction)
    ...
    def _finish(self) -> None:
        ...
        self.net_change_amt = self._new_add.amount - self._cancelled.amount - self._executed.amount
        if self._new_add.count:
            ...
            self.inflow_streak = 0
        had_records = bool(self._records and self._current.count)
        self.limit_up_may_gone = had_records and (self._cancelled.amount + self._executed.amount) * 5 > self._current.amount
        self.limit_up_gone = bool(self._records) and self._current.count == 0
        for entry in self._my_orders:
            if entry.found_id:
...
class _D202WebSocketSource:
    ...
    def connect(self, block: bool = False) -> None:
        ...
        runner = lambda: self._ws.run_forever(ping_interval=30, ping_timeout=10)
        if block:
            runner()
        else:
    ...
    def _on_message(self, _ws: Any, text: str) -> None:
        ...
        by_code = {str(item.get("code") or ""): item for item in self._watchers}
        ...
        with self._lock:
            ...
            if self._watcher_factory is None or self._source_factory is None:
                try:
                    ...
                    # The upstream repository currently ships an invalid
                    # setuptools backend declaration. Keep the protocol
                    # adapter usable with the project's websocket-client
                    # dependency while retaining the same D202 wire format.
                    logger.info("使用内置 D202 兼容实现: %s", exc)
                    self._watcher_factory = _D202Watcher
                    self._source_factory = _D202WebSocketSource
                else:
                    self._watcher_factory = LimitUpWatcher
                    self._source_factory = WebSocketSource
            self._state = "idle"
        ...
        with self._lock:
            ...
            signature = {
                symbol: (
                    ...
                    str(spec.get("queue_key") or ""),
                    int(spec.get("queue_volume") or 0),
                )
    ...
    def _restart_locked(self) -> None:
        ...
        try:
            ...
            for symbol, spec in self._specs.items():
                ...
                watcher = self._watcher_factory(code, price_li=int(round(price * 1000)))

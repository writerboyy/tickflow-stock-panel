...
`)
  ...
  const loadTemplate = (id: string) => {
    ...
    setSelectedId('')
    setConfig(prev => ({ ...prev, strategy_id: '' }))
  }
  ...
  const run = async () => {
    ...
    try {
      ...
      events.onmessage = event => {
        ...
        if (payload.type === 'result') { setResult(payload.result); setProgress('回测完成'); setRunning(false); events.close() }
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)_280px] gap-3 max-xl:grid-cols-[180px_minmax(0,1fr)]">
      ...
      <section className="min-h-0 overflow-y-auto rounded-md border border-border bg-surface p-3 max-xl:col-span-2">
    ...
    {result && <section className="shrink-0 rounded-md border border-border bg-surface p-3"><div className="mb-3 flex items-center justify-between"><span className="text-xs font-medium">回测结果</span><span className="text-[10px] text-muted">{result.metadata?.nav_filter === 'skipped_no_data' ? 'NAV/溢价过滤：无数据已跳过' : ''}</span></div><div className="grid grid-cols-4 gap-4"><Metric label="期末净值" value={result.final_equity.toLocaleString('zh-CN', { maximumFractionDigits: 2 })} /><Metric label="收益率" value={`${result.return_pct.toFixed(2)}%`} /><Metric label="最大回撤" value={`${result.max_drawdown_pct.toFixed(2)}%`} /><Metric label="成交笔数" value={String(result.fills.length)} /></div><div className="mt-3 grid gap-3 lg:grid-cols-2"><div className="overflow-x-auto"><div className="mb-1 text-[11px] font-medium">成交</div><table className="w-full min-w-[520px] text-[11px]"><thead className="text-left text-muted"><tr><th className="pb-1">时间</th><th className="pb-1">标的</th><th className="pb-1">方向</th><th className="pb-1">数量</th><th className="pb-1">价格</th><th className="pb-1">费用</th></tr></thead><tbody>{result.fills.slice(-20).map((fill, index) => <tr key={`${fill.order_id}-${index}`} className="border-t border-border"><td className="py-1">{fill.timestamp}</td><td>{fill.symbol}</td><td>{fill.side}</td><td>{fill.quantity}</td><td>{Number(fill.price).toFixed(3)}</td><td>{Number(fill.fee).toFixed(2)}</td></tr>)}</tbody></table></div><div className="grid gap-3 sm:grid-cols-2"><div><div className="mb-1 text-[11px] font-medium">当前持仓</div><div className="space-y-1 text-[11px] text-secondary">{Object.entries(result.positions).filter(([, qty]) => qty > 0).map(([symbol, qty]) => <div key={symbol} className="flex justify-between border-b border-border py-1"><span>{symbol}</span><span className="tabular-nums">{qty}</span></div>)}{!Object.values(result.positions).some(qty => qty > 0) && <div className="text-muted">无持仓</div>}</div></div><div><div className="mb-1 text-[11px] font-medium">策略日志</div><div className="max-h-28 space-y-1 overflow-auto text-[10px] text-secondary">{result.logs.slice(-12).map((log, index) => <div key={`${log.timestamp}-${index}`}><span className="text-muted">{log.timestamp.slice(0, 16)}</span> {log.message}</div>)}{!result.logs.length && <div className="text-muted">无策略日志</div>}</div></div></div></div><div className="mt-3 overflow-x-auto"><div className="mb-1 text-[11px] font-medium">逐 bar 资产</div><table className="w-full min-w-[520px] text-[11px]"><thead className="text-left text-muted"><tr><th className="pb-1">时间</th><th className="pb-1">总资产</th><th className="pb-1">现金</th><th className="pb-1">仓位数</th></tr></thead><tbody>{result.equity_curve.slice(-12).map(row => <tr key={row.timestamp} className="border-t border-border"><td className="py-1">{row.timestamp}</td><td>{row.equity.toLocaleString('zh-CN', { maximumFractionDigits: 2 })}</td><td>{row.cash.toLocaleString('zh-CN', { maximumFractionDigits: 2 })}</td><td>{Object.keys(row.positions).length}</td></tr>)}</tbody></table></div></section>}
  </div>
}

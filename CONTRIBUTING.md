...
- 分钟 K 时间标记遵循左开右闭：例如 `09:31:00` 表示 `(09:30:00, 09:31:00]` 内的成交；`09:30:00` 为例外，单独一根表示集合竞价。
- 分钟 K 的股票、ETF、指数分开存储和路由，不得仅凭代码格式猜测资产类型。
- TickFlow 历史分钟 K 采用稀疏表示：临时停牌或无成交时段可能省略零成交分钟。单日少于 240 根，或首根晚于 `09:31:00`，不能单独作为行情缺失的结论。
- 判断分钟数据问题前，必须直接重拉 TickFlow 原始响应，并结合成交量、停复牌记录和交易时段核对。只有缺少真实成交、时间戳异常、OHLC/单位错误或同一成交键不一致时，才归类为数据问题。
- Tushare 等来源提供的零成交分钟是时钟占位数据，不证明该时段发生了交易。此类行可用于构造固定 240 根时钟网格，但不得覆盖 TickFlow 已有成交键，也不得据此指控 TickFlow 丢失成交。
- 固定网格补齐与数据修复必须分开记录：补齐行保持成交量和成交额为零，并在成交模拟中视为不可成交；审计报告应标明原始行情采用稀疏表示。
...
- 用户提出或研究产出的可执行策略，默认保存到「自定义 → 量化策略」工作区，对应 `data/free_strategies/<strategy_id>/strategy.py`；`/backtest` 页面只用于验证策略和查看回测结果，不作为策略源码的默认归档位置。只有用户明确要求选股卡片、策略扫描或策略监控时，才使用 `data/strategies/custom/*.py` 的选股策略契约。
...
- 同一批调仓必须先成交卖出和减仓委托，再让买入委托按实际递减的可用现金成交；多目标买入使用引擎现金权重委托统一分配，份额包含手续费。策略不得预估待成交卖出收入，不得复制费率或滑点计算，任何配置下账户现金都不得因买入变为负数。
...
- 外部策略存在参考成交明细时，回测必须通过 provider 和标准行情仓库读取行情，并在完整参考区间逐笔核对成交时间、标的、方向、数量、价格、金额和费用；不得用参考 CSV 反向生成行情或成交结果。
- `execution_mode` 表示策略回放方式，不表示行情来源：定义 `on_bar` 并逐根消费 K 线的策略使用 `full_bar`，仅定时回调的策略使用 `scheduled`；模拟行情周期由 `market_mode` 表示，实际数据来源和存储由回测元数据记录。
- 移植策略的回测和模拟本金默认统一为 `100,000` 元，与五福策略保持一致。确需不同本金时必须有明确需求，并同时验证参考成交和模拟配置，不能用额外闲置资金掩盖手续费或现金约束差异。
- 创建模拟盘前必须持久化已验证的回测结果，并保证策略 ID、源码哈希和全部执行参数一致；通过历史检查点续接净值、持仓和策略状态后再启动模拟。除非用户明确要求从零开始，不得把空账户直接切到实时行情。
- 自动创建模拟账户时，账户名称直接使用策略名称，不追加“模拟”“模拟盘”等环境标识。
- 交付时分别报告回测的 `execution_mode`、数据覆盖与逐笔对账结果，以及模拟盘的 `market_mode`、续接来源、同步阶段和运行状态，避免把“进程已启动”误报为历史状态已接入。
...
- 新增或修改确认、删除、清空等操作必须使用系统 `Modal` 组件，禁止调用浏览器原生 `window.alert`、`window.confirm` 或 `window.prompt`，以保证主题、焦点管理、无障碍和窄屏可操作性。

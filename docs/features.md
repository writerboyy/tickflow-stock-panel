...
盘后管道(15:30 CST 自动触发)会重新拉日 K、重算 enriched 表，并增量生成
`valuation_daily` 日度估值表。该表按交易日分区，以未复权收盘价、历史股本和公告日
PIT 财务报表统一生成总市值、流通市值、PE-TTM、PB、PS-TTM 和 PCF-TTM。
策略只读取这张持久化表，不读取或回退到外部 `financials/valuation`，也不在策略内
临时推算市值；缺少输入时对应指标保持为空。
...
- **告警边界**:监控中心触发记录、未读徽标和公共实时提醒只接受 `MonitorRuleEngine` 的规则来源(`strategy`/`signal`/`price`/`market`/`ladder`/`sector`)；打板专区、持仓风控等领域服务不能绕过监控规则写入公共告警
...
定时盘后管道完成后会后台增量同步财务数据：已有标的只拉最新公告，新标的自动补全历史；无财务权限或已有同步任务时跳过，不阻塞行情管道。

APScheduler 15:30 CST 自动:拉日 K → 重算 enriched 表 → 更新日度估值 → 跑监控规则。

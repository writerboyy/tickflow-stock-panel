...
设置页的主数据源切换列表。它们通过标准扩展表为策略补充维度，不改变行情路由：

| 来源 | 本系统使用范围 | 不重复采集的数据 |
| --- | --- | --- |
| TickFlow | 行情、证券维表、股本、财务 | - |
| 开盘啦 | 竞价、涨停复盘、龙虎榜、监管快照 | 行情、股本、财务、行业 |
| EasyTDX | `industry_sw`、`industry_tdx` | K 线、实时行情、名称、股本、财务、题材及开盘啦已有数据 |

EasyTDX 是可选依赖，具体安装、快照契约、调度和失败行为见
[数据源插件开发指南](./plugin-development.md#easytdx-行业维度)。

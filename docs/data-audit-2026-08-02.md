...
整改后当前状态：前 3 组 P1 已完成代码修复和生产重算；股票 enriched、valuation 仍各为 6,160,370 个唯一键，EasyTDX 分红 7,259 行差异已归零并从 4,005 个文件压缩为 27 个年度文件，股票分钟 149,179 行空 OHLC 已清除。严格强化开盘啦归档回放后又发现龙虎榜席位明细旧主键会覆盖同股同方向不同上榜原因的记录，现已将主键升级为 `(symbol,side,log_id)` 并从原始归档恢复 78 行，720 行修复为 798 行。指数异常仍存在：同源在线重拉的 8,477 行与本地逐字段一致，远端本身仍返回异常值；新增的异常后 EasyTDX 只读核验覆盖 825/835 行，确认 485 行 TickFlow 异常、发现 340 行源间冲突，另有 10 行无通达信数据。BaoStock 将成交量从股除以 100 转为手后覆盖 187 个异常键，全部与 EasyTDX 一致；其余 648 个异常键无 BaoStock 数据。因此现行入口继续拒绝发布，不用核验源静默改写。财务 8 组冲突也仍存在，当前账号没有 TickFlow financial 能力，无法取得 revision 证据，禁止按文件顺序猜测修复。分钟历史覆盖不足、指数分钟缺失和 depth5 历史不可回补仍是能力边界。
...
| `kline_index_daily` | 日频；`symbol,date`；按日分区 | 704,552 行，`2021-08-02..2026-07-31` | TickFlow 指数行情；指数看板/市场背景 | **不通过**：835 行命中质量门禁，其中 495 行负成交量，`000691.SH` 有 638 行成交额异常 |
...
- 指数负成交量为 495 行、7 个 symbol：`000011.SH` 5 行、`000691.SH` 298 行、`000902.SH` 6 行、`000985.SH` 4 行、`399317.SZ` 6 行、`399379.SZ` 93 行、`399380.SZ` 83 行。`000691.SH` 的 638 行异常成交额中有 298 行与负量重叠，因此质量门禁合计命中 835 个唯一键，范围为 `2021-08-02..2026-07-30`。
...
- EasyTDX 指数日线只在 TickFlow 指数质量门禁失败后用于只读核验；BaoStock 只用于本报告的一次性第三源复核。两者均不写 canonical 行情，也不构成自动修复来源。
...
- `tickflow_anomaly_confirmed`：EasyTDX 自身合法，且 TickFlow 未被门禁判错的字段在容差内一致；
- `source_conflict`：EasyTDX 自身合法，但其他可比较字段与 TickFlow 冲突；
- `both_anomalous`：EasyTDX 同样不通过同一质量规则；
- `tdx_unavailable`：EasyTDX 无对应键、依赖不可用或请求失败。

EasyTDX 实际核验 825/835 个键：485 个 `tickflow_anomaly_confirmed`、340 个 `source_conflict`、10 个 `tdx_unavailable`，10:27 复跑计数完全一致。独立 CLI 只在显式指定时把完整证据写到 `/tmp`，不调用 collector、不写行情表；本服务只记录汇总，不把供应商响应或本地绝对路径写入日志。

BaoStock 使用临时依赖对 7 个指数查询相同区间的未复权日线。其指数 `volume` 是股，必须显式除以 100 后再与 EasyTDX/TickFlow 的手口径比较；价格和成交额不做启发式缩放。结果如下：

| 指数 | 门禁异常键 | EasyTDX 结果 | BaoStock 覆盖 | 三源结论 |
| --- | ---: | --- | ---: | --- |
| `000011.SH` | 5 | 确认 5 | 5 | EasyTDX 与 BaoStock 全部一致，共同确认 TickFlow 异常 |
| `000691.SH` | 638 | 确认 298、源冲突 340 | 0 | BaoStock 无该指数数据；298 行只有 EasyTDX 单源确认，340 行仍为源冲突 |
| `000902.SH` | 6 | 无数据 6 | 0 | 两个辅助源均无数据，不可进一步验证 |
| `000985.SH` | 4 | 无数据 4 | 0 | 两个辅助源均无数据，不可进一步验证 |
| `399317.SZ` | 6 | 确认 6 | 6 | EasyTDX 与 BaoStock 全部一致，共同确认 TickFlow 异常 |
| `399379.SZ` | 93 | 确认 93 | 93 | EasyTDX 与 BaoStock 全部一致，共同确认 TickFlow 异常 |
| `399380.SZ` | 83 | 确认 83 | 83 | EasyTDX 与 BaoStock 全部一致，共同确认 TickFlow 异常 |
...
未完成目标由外部证据不足造成，而不是核验未执行：`000691.SH` 的 340 个冲突键需要第三个有覆盖且口径可证明的指数源或 TickFlow revision；`000902.SH`、`000985.SH` 的 10 个键需要可覆盖该指数的参考源。即使 187/298 个键已确认异常，也仍缺少 TickFlow 可审计 revision 和全批次修复值，故数据湖保持原值、异常批次保持 fail-closed；取得 revision 后应先影子修复 `kline_index_daily`，逐键复核，再重建 `kline_index_enriched` 和依赖缓存。
...
| 阻塞 P1 | 指数负量/异常成交额 | 入口已 fail-closed，并在异常后执行 EasyTDX 只读核验；187 行获 EasyTDX+BaoStock 双重确认、298 行获 EasyTDX 单源确认，340 行源冲突、10 行无辅助覆盖。等待 TickFlow revision 或补充可覆盖参考源，不能本地猜值 |
...
- 财务 8 组冲突和指数源异常仍保留在数据湖中；指数的 350 个键仍因源冲突或双辅助源无覆盖而不可定论，已确认异常的键也缺少可审计 revision，不能直接用核验值覆盖。其余“已解决”项均有生产 repair manifest、source snapshot 或回滚目录证明，不能仅以测试通过替代数据验收。
...
| 指数在线核源 | 7 个指数、8,477 行在线 TickFlow 与本地逐 OHLCVA 字段一致；835 个门禁异常键再由 EasyTDX 核验 825 个，BaoStock 标准化单位后覆盖 187 个且全部与 EasyTDX 一致；本地异常未改 | 340 个源冲突、10 个双辅助源无覆盖；等待上游 revision 或补充可覆盖参考源，不得用辅助源静默替代 |
